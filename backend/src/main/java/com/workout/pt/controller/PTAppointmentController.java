...
public class PTAppointmentController {
  ...
  /**
   * 1. PT 스케줄 생성 (수업 예약) - 사용자와 트레이너 모두 예약할 수 있어야 합니다. - 요청 Body에 어떤 계약(contract_id)에 대한 예약인지
   * 명시합니다.
   */
  @PostMapping
  public ResponseEntity<Void> createAppointment(
      ...
      // DTO: { "contractId": 101, "startTime": "...", "endTime": "..." }
  ) {
  ...
  /**
   * 2. PT 스케줄 조회 - 이 API는 역할(사용자인지, 트레이너인지)에 따라 다른 Controller에 있을 수도 있습니다. (아래 추가 설명 참고) - 특정 예약 한
   * 건에 대한 상세 조회입니다.
   */
  ...
  /**
   * 3. PT 스케줄 수정 (시간 변경) - 예약 시간을 변경합니다.
   */
  ...
  /**
   * 4. PT 스케줄 상태 변경 (e.g., 완료, 취소) - 수업을 '완료(COMPLETED)' 처리하거나 '취소(CANCELED)' 합니다.
   */
